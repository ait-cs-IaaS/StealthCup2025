#resource "aws_instance" "enterprise_dragos" {
#  ami             = var.images["dragos"]
#  instance_type   = var.flavors["dragos"]
...
#  tags = {
#    Name = "${var.team}_monitoring_dragos"
...
resource "aws_instance" "monitoring_dragos" {
  ami = lookup(
    lookup(var.team_images, var.team, {}), 
    "monitoring_dragos", 
    var.df_team_images["build"]["monitoring_dragos"]
  )
  instance_type   = var.flavors["dragos"]
  key_name        = var.ssh_key
  ...
  network_interface {
    network_interface_id = aws_network_interface.monitoring_dragos_primary_nic.id
    device_index         = 1
  }

  network_interface {
    network_interface_id = aws_network_interface.monitoring_dragos_secondary_nic.id
    device_index         = 0
  }

  tags = {
    Name   = "${var.team}_monitoring_dragos"
    Groups = "monitoring,dragos,${var.team}"
  }
}


# Primary Network Interface (First NIC)
resource "aws_network_interface" "monitoring_dragos_primary_nic" {
  subnet_id       = aws_subnet.enterprise_server_subnet.id
  ...
  tags = {
    Name = "${var.team}_monitoring_dragos_1_primary_nic"
  }
}

resource "aws_network_interface" "monitoring_dragos_secondary_nic" {
  subnet_id       = aws_subnet.enterprise_server_subnet.id
  ...
  tags = {
    Name = "${var.team}_monitoring_dragos_1_secondary_nic"
  }
}
...
#resource "aws_lb_target_group_attachment" "dragos_attachment" {
#  target_group_arn = aws_lb_target_group.traffic_mirror_tg.arn
#  target_id        = aws_instance.monitoring_dragos.id
#  port             = 4789
#}
