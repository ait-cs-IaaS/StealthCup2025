...
def restart_instance(instance: str, rootpath: str, vm_map: Dict[str, str]) -> None:
    ...
    ### TODO adapt to AWS
    print(f"Resource: aws_instance.{instance}")
    print(f"Path: {rootpath}/terragrunt/{subdir}")
    os.chdir(os.path.join(rootpath, "terragrunt", subdir))
    #### TODO adapt this to AWS, image name? ami number?
    p = subprocess.run(['terragrunt', 'destroy', '--target', f"aws_instance.{instance}"])
...
def main():
    parser = argparse.ArgumentParser(description="Redeploy Instances")
    parser.add_argument(
        "yaml_file", 
        type=str, 
